...
export default async function PortalLayout({ children }: { children: React.ReactNode }) {
  ...
  let profile = { role: metaRole, full_name: metaName };
  
  try {
    const { data } = await supabase.from('profiles').select('role, full_name').eq('id', user.id).single();
    if (data) {
      if (data.role === 'student' && user.user_metadata?.role && user.user_metadata.role !== 'student') {
        profile = { ...data, role: user.user_metadata.role };
      } else {
        profile = data;
      }
    }
